...
void mem_op(struct EX_MEM* EX_MEM, struct MEM_WB* MEM_WB, int f_val){
    // Pass Stage Regs Info
	...
	if(EX_MEM->WB_SIG){		// load
		...
		MEM_WB->MEM_OUT = Memory::instance().readMem(EX_MEM->ALU_OUT);
	    else{			// LB
		MEM_WB->MEM_OUT = Memory::instance().readMem(EX_MEM->ALU_OUT, 1);
	    ...
	    if(EX_MEM->MEM_SIG==1){	// SW
		    ...
		    Memory::instance().writeMem(val, EX_MEM->ALU_OUT);
	    } else {			// SB
	            if(EX_MEM->Rt < 0)
			    val = f_val;
		    else
			    val = Memory::instance().readMem(EX_MEM->Rt, 1);
		    Memory::instance().writeMem(val&0xFF, EX_MEM->ALU_OUT, 1);
	    }
	}
    }
    return;
